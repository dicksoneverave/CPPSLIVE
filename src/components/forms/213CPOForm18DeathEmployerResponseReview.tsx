...
const Form124CPOForm18DeathEmployerResponseReview: React.FC<DeathReviewProps> = ({ 
  irn, 
  incidentType, 
  onClose, 
  onSubmit, 
  ...
  const handleForwardToWorker = async () => {
    ...
    try {
      ...
      await downloadForm18CPO(irn, {
        logoPath: 'https://ennhknwwfdlaudephyly.supabase.co/storage/v1/object/public/cpps/logocrest.png'
      });
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-gray-900">
          Form 18 - Employer Accepted (Death)
        </h2>
        <button
          onClick={onClose}
      ...
      {showSuccess && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[60]">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 text-center">
            ...
            <button onClick={closeAll} className="px-6 py-2 bg-primary text-white rounded-md hover:bg-primary-dark">
              Close
            </button>
          </div>
        </div>
