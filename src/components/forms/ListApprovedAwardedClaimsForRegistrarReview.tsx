...
import ApprovedAwardedClaimsForRegistrarReview from './ApprovedAwardedClaimsForRegistrarReview'; // ← adjust path if needed
...
const ListApprovedAwardedClaimsForRegistrarReview: React.FC<ListApprovedAwardedClaimsForRegistrarReviewProps> = ({ 
  ...
  // NEW: state to show the review component
  ...
  const fetchClaimsList = async () => {
    try {
      ...
      let query = supabase
        ...
        .order('DecisionDateFormatted', { ascending: false });
  ...
  // UPDATED: open the review component here (and still support optional callback)
  const handleProceed = (irn: string, incidentType: string) => {
    if (onSelectIRN) {
      onSelectIRN(irn, incidentType);
      return;
    }
    const formType: 'Injury' | 'Death' = incidentType === 'Death' ? 'Death' : 'Injury';
    setSelectedIRN(irn);
    setSelectedIncidentType(formType);
    setShowReview(true);
  };
  ...
  return (
    <>
      ...
      {/* NEW: reviewer modal layered above (z-index handled by its own component) */}
      {showReview && (
        <ApprovedAwardedClaimsForRegistrarReview
          IRN={selectedIRN}
          formType={selectedIncidentType}
          onCloseAll={() => setShowReview(false)}
        />
      )}
    </>
  );
