...
import PendingForm6ForInsuranceProviderReview from './PendingForm6ForInsuranceProviderReview';

interface ListPendingForm6ForInsuranceProviderReviewProps {
  onClose: () => void;
  onSelectIRN?: (irn: string, incidentType: 'Injury' | 'Death') => void;
  // NEW (optional): notify parent to refresh/remove the processed IRN from any outer list
  onRemovedIRN?: (irn: string) => void;
...
const ListPendingForm6ForInsuranceProviderReview: React.FC<ListPendingForm6ForInsuranceProviderReviewProps> = ({
  onClose,
  onSelectIRN,
  onRemovedIRN,
  onRefreshParent,
  ...
  const fetchList = async () => {
    try {
      ...
      let query = supabase
        .from('v_form6_insurance_provider_review')
        .select(
      ...
      setRows(
        (data || []).map((d: any) => ({
          irn: d.IRN,
          displayirn: d.DisplayIRN,
          workerfirstname: d.WorkerFirstName,
          workerlastname: d.WorkerLastName,
          f6mapprovaldate: d.F6MApprovalDate,
          incidenttype: d.IncidentType,
          organizationname: d.OrganizationName,
          f6mstatus: d.F6MStatus,
        }))
      );
  ...
  // NEW: Called by detail modal after successful Proceed → OK
  const handleDetailSuccess = async (processedIrn: string) => {
    // Optimistically remove from local list
    setRows(prev => prev.filter(r => String(r.irn) !== processedIrn));
    setTotalRecords(prev => Math.max(0, prev - 1));

    // If page became empty and not the first page, go back a page
    if (rows.length === 1 && currentPage > 1) {
      setCurrentPage(p => p - 1);
    } else {
      // Otherwise, refresh list to keep counts/ordering accurate
      await fetchList();
    }

    // Notify parent (dashboard) if it passed hooks
    onRemovedIRN?.(processedIrn);
    onRefreshParent?.();
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      ...
      {showPendingForm6 && selectedIRN && (
        <PendingForm6ForInsuranceProviderReview
          irn={selectedIRN}
          incidentType={selectedIncidentType}
          onClose={closeModal}
          // NEW: when the inner form saves successfully and user clicks OK
          onSuccess={() => handleDetailSuccess(selectedIRN)}
        />
      )}
    </div>
  );
...
export default ListPendingForm6ForInsuranceProviderReview;
