...
interface TribunalHearingPublicProps {
  onClose: () => void;
  onSelectIRN?: (irn: string, action: string, hearingType?: string, hearingNo?: string) => void;
...
const ListSetTribunalHearingPublic: React.FC<TribunalHearingPublicProps> = ({ 
  ...
  useEffect(() => {
    fetchHearingsList();
  }, [currentPage, searchIRN, searchFirstName, searchLastName, refreshKey]);
  ...
  const fetchHearingsList = async () => {
    try {
      ...
      if (mode === 'completion') {
        countQuery = countQuery.eq('THSSetForHearing', 'Scheduled');
      }
      ...
      if (mode === 'completion') {
        query = query.eq('THSSetForHearing', 'Scheduled');
      }
      ...
      const formattedData = hearingData.map(item => {
        ...
        return {
          ...
          HearingNo: item.THSHearingNo || 'N/A'
        };
      });
  ...
  const handleAction = async (irn: string, setForHearing: string, hearingType?: string, hearingNo?: string) => {
    const action = setForHearing === 'Not Scheduled' ? 'Schedule' : (mode === 'completion' ? 'Record Outcome' : 'View');
    
    if (onSelectIRN) {
      onSelectIRN(irn, action, hearingType, hearingNo);
      return;
    }
    ...
    if (setForHearing === 'Scheduled') {
      ...
      if (hearing) {
        setSelectedIRN(irn);
        setSelectedHearingType(hearing.Type || '');
        ...
        switch (hearing.Type) {
          ...
          case 'Form7EmployerRejectedOtherReason':
            setShowForm253(true);
            break;
          case 'Form6StateSolicitorSumbission':
            try {
        ...
        <div className="flex items-center justify-between p-4 border-b sticky top-0 bg-white">
          <div className="flex items-center">
            ...
            <h2 className="text-xl font-semibold text-gray-900">
              {mode === 'completion' ? 'Tribunal Hearing - Enter Outcome (Public)' : 'Tribunal Hearing (Public)'}
            </h2>
          </div>
                  ...
                  {hearingsList.map((hearing, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button
                          onClick={() => handleAction(hearing.IRN, hearing.SetForHearing, hearing.Type, hearing.HearingNo)}
                          className="text-sm font-medium bg-primary hover:bg-primary-dark text-white px-3 py-1 rounded"
                        >
                          {hearing.SetForHearing === 'Not Scheduled' ? 'Schedule' : (mode === 'completion' ? 'Record Outcome' : 'View')}
                        </button>
                      </td>
                    </tr>
                  ))}
