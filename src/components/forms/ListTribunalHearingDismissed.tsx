...
const ListTribunalHearingDismissed: React.FC<ListTribunalHearingDismissedProps> = ({ 
  ...
  const [appealIRN, setAppealIRN] = useState<string | null>(null);
  ...
  const generateCoverLettersPDF = async () => {
    try {
      ...
      for (let i = 0; i < consentedList.length; i++) {
        ...
        doc.text('DISMISS CLAIM - COVER LETTER', pageWidth / 2, y, { align: 'center' });
        y += 20;
        ...
        // Subject
        doc.setFont('times', 'bold');
        doc.text('Subject: Tribunal Decision', margin, y);
        y += 10;
        ...
        const body1 = `We refer to your workers' compensation claim, which was reviewed during the recent Workers' Compensation Tribunal Hearing held in ${venue}, ${location}, ${dateStr}.`;
        const body2 = `Following the tribunal’s deliberation, the State has denied liability, and your claim has been dismissed. The reasons for this decision are outlined in the attached Record of Proceedings (ROP).`;
        const body3 = `Should you wish to appeal the tribunal’s decision, you may do so by filing an appeal with the National Court within 30 days from the date of receipt of this letter.`;
        const body4 = `For further information or assistance, please do not hesitate to contact our office or your nearest Provincial Labour Office.`;
        ...
        const split4 = doc.splitTextToSize(body4, pageWidth - margin * 2);
      ...
      doc.save('Dismiss-Cover-Letters.pdf');
    } catch (err: any) {
    ...
    try {
      ...
      for (let i = 0; i < consentedList.length; i++) {
        ...
        if (outcomeData?.THOReason) {
          ...
          doc.text('Dismissed', margin, yPosition);
          yPosition += 10; // Initially 15
        }
  ...
  const confirmAppeal = async () => {
    ...
    try {
      ...
      // 1. Update tribunalhearingschedule
      const { error: scheduleError } = await supabase
        .from('tribunalhearingschedule')
        .update({
          THSSubmissionDate: new Date().toISOString(),
          THSSetForHearing: 'Appealed',
      ...
      // 2. Delete from tribunalhearingoutcome
      ...
      setAppealIRN(null);
      fetchConsentedList(); // Refresh list
      
    } catch (err: any) {
      console.error('Error appealing hearing:', err);
      setError(`Failed to process appeal: ${err.message}`);
    } finally {
                    ...
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex space-x-2">
                          ...
                          <button
                            onClick={() => handleAppeal(hearing.THOIRN)}
                            className="text-sm font-medium bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded"
                          >
                            Appeal
                          </button>
                        </div>
      ...
      {showAppealModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-[60]">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
            <h3 className="text-xl font-semibold mb-4 text-gray-900">Confirm Appeal</h3>
            <p className="text-gray-600 mb-6">
              Are you sure you want to re-instate this Claim for re-hearing? 
              This will move the record back to the pending hearing list.
            </p>
