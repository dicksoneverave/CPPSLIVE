...
import { X, Search, Plus, Edit2, Trash2, Eye, Save, ArrowLeft, AlertTriangle, Accessibility } from 'lucide-react';
...
interface OWCBodyPartManagerProps {
  onClose: () => void;
  initialView?: 'list' | 'add' | 'edit' | 'view';
}

const OWCBodyPartManager: React.FC<OWCBodyPartManagerProps> = ({ 
  onClose,
  initialView = 'list'
}) => {
  ...
  const [selectedItem, setSelectedItem] = useState<BodyPart | null>(null);
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<BodyPart | null>(null);
  ...
  const handleEdit = (item: BodyPart) => {
  ...
  const handleView = (item: BodyPart) => {
    setSelectedItem(item);
    setFormData({
      DKey: item.DKey || 'Name',
      DValue: item.DValue || ''
  ...
  const confirmDelete = (item: BodyPart) => {
        ...
        <div className="flex items-center justify-between p-6 border-b bg-white">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-gray-900">
              {view === 'list' && 'Body Part Management'}
              {view === 'add' && 'Add New Body Part'}
              {view === 'edit' && 'Edit Body Part'}
              {view === 'view' && 'View Body Part Details'}
            </h2>
          </div>
            ...
            <div className="flex-1 flex flex-col min-h-0 p-6 space-y-4 overflow-hidden">
              <div className="flex items-center justify-between gap-4">
                <div className="relative flex-1">
                  ...
                  <input
                    type="text"
                    placeholder="Search by body part name..."
              ...
              <div className="flex-1 overflow-auto border border-gray-200 rounded-lg shadow-sm">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Body Part Name</th>
                      ...
                      <tr>
                        <td colSpan={3} className="px-6 py-10 text-center text-gray-500">
                          <div className="flex flex-col items-center gap-2">
                            ...
                            <span>Loading body parts...</span>
                          </div>
                        </td>
                      </tr>
                    ) : filteredData.length === 0 ? (
                      <tr>
                        <td colSpan={3} className="px-6 py-10 text-center text-gray-500">No body parts found.</td>
                      </tr>
                    ) : filteredData.map((item) => (
                      <tr key={item.ID} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{item.DValue}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">{item.DKey}</td>
            ...
            <div className="flex-1 flex flex-col min-h-0 overflow-y-auto p-6 space-y-6">
              ...
              <form onSubmit={handleSubmit} className="max-w-2xl mx-auto w-full space-y-6 bg-gray-50 p-8 rounded-xl border border-gray-100">
                <div className="grid grid-cols-1 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Body Part Name</label>
                    <input
                      type="text"
                      name="DValue"
                ...
                <div className="flex justify-end gap-3 pt-4">
                  ...
                  {view !== 'view' && (
                    ...
                    >
                      ...
                      {view === 'add' ? 'Save Body Part' : 'Update Body Part'}
                    </button>
                  )}
                </div>
              </form>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 transform animate-in zoom-in-95 duration-200">
            <div className="flex items-center gap-4 text-red-600 mb-4">
              ...
              <h3 className="text-xl font-bold">Delete Body Part?</h3>
            </div>
...
export default OWCBodyPartManager;
