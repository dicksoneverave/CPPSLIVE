...
import { X, Search, Plus, Edit2, Trash2, Eye, Save, ArrowLeft, AlertTriangle, Settings } from 'lucide-react';
...
interface OWCSystemParameterManagerProps {
  onClose: () => void;
  initialView?: 'list' | 'add' | 'edit' | 'view';
}

const OWCSystemParameterManager: React.FC<OWCSystemParameterManagerProps> = ({ 
  onClose,
  initialView = 'list'
}) => {
  ...
  const handleView = (param: SystemParameter) => {
    setSelectedParameter(param);
    setFormData({
      DKey: param.DKey || '',
      DValue: param.DValue || ''
    ...
    try {
      ...
      if (view === 'add') {
        const { error } = await supabase
          ...
          .insert([payload]);
        if (error) throw error;
      } else if (view === 'edit' && selectedParameter) {
        const { error } = await supabase
          .from('dictionary')
          .update(payload)
  ...
  const confirmDelete = (param: SystemParameter) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        ...
        <div className="flex items-center justify-between p-6 border-b bg-white">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-gray-900">
              ...
              {view === 'add' && 'Add New Parameter'}
              {view === 'edit' && 'Edit System Parameter'}
              {view === 'view' && 'View Parameter Details'}
            </h2>
          </div>
            ...
            <div className="flex-1 flex flex-col min-h-0 p-6 space-y-4 overflow-hidden">
              <div className="flex items-center justify-between gap-4">
                <div className="relative flex-1">
                  ...
                  <input
                    type="text"
                    placeholder="Search by parameter key or value..."
              ...
              <div className="flex-1 overflow-auto border border-gray-200 rounded-lg shadow-sm">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Parameter Key</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {loading ? (
                      ...
                      <tr>
                        <td colSpan={3} className="px-6 py-10 text-center text-gray-500">No system parameters found.</td>
                      </tr>
                    ) : filteredParameters.map((p) => (
                      <tr key={p.ID} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 font-mono">{p.DKey}</td>
                        <td className="px-6 py-4 text-sm text-gray-500 max-w-md truncate">{p.DValue}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                          ...
                          <button onClick={() => handleEdit(p)} title="Edit Parameter" className="text-blue-400 hover:text-blue-600 transition-colors">
                            <Edit2 className="h-5 w-5" />
                          </button>
                          <button onClick={() => confirmDelete(p)} title="Delete Parameter" className="text-red-400 hover:text-red-600 transition-colors">
            ...
            <div className="flex-1 flex flex-col min-h-0 overflow-y-auto p-6 space-y-6">
              ...
              <form onSubmit={handleSubmit} className="max-w-2xl mx-auto w-full space-y-6 bg-gray-50 p-8 rounded-xl border border-gray-100">
                <div className="grid grid-cols-1 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Parameter Key / Name</label>
                    <input
                      type="text"
                      name="DKey"
                      placeholder="e.g. SYSTEM_TIMEOUT"
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none disabled:bg-gray-100 font-mono font-bold"
                      value={formData.DKey}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none disabled:bg-gray-100 resize-none"
                ...
                <div className="flex justify-end gap-3 pt-4">
                  ...
                  {view !== 'view' && (
                    ...
                    >
                      ...
                      {view === 'add' ? 'Save Parameter' : 'Update Parameter'}
                    </button>
                  )}
                </div>
              </form>
      ...
      {showDeleteConfirm && parameterToDelete && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 transform animate-in zoom-in-95 duration-200">
            ...
            <p className="text-gray-600 mb-6 leading-relaxed">
              Are you sure you want to delete the system parameter 
              <span className="font-bold text-gray-900 ml-1 font-mono">
                "{parameterToDelete.DKey}"
              </span>? 
              This could affect system behavior and cannot be undone.
            </p>
...
export default OWCSystemParameterManager;
