...
const PendingClaimsAwardedForPaymentsManagerReview: React.FC<Props> = ({
  IRN,
  irn,
  formType = 'Injury',
  onCloseAll,
  ...
  // who am I?
  // NEW: Multi-recipient split records
  const [splitRecords, setSplitRecords] = useState<any[]>([]);

  const [paymentManagerReviewStatus] = useState<string>('');
  const [batchNo] = useState<string>('');
  ...
  // Try to load the saved deposit row for this IRN
  useEffect(() => {
    (async () => {
  ...
  const handleProceedDecision = async (e?: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const reviewStatus = statusMap[decision as Decision];
      ...
      // 2) Update bankaccountdepositmaster (ALL records for this IRN)
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4" onClick={handleClose}>
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 border-b">
            <div className="flex items-center gap-3">
              <h2 className="text-lg font-semibold text-gray-900">
                Payments Manager Review (Pending) — {formType}
              </h2>
          ...
          <div className="p-6 space-y-8 overflow-y-auto">
            ...
            {/* Section 4: Breakdown and Decision */}
            <div className="space-y-6">
              ...
              <div className="mt-6 grid md:grid-cols-2 gap-4 text-xs text-gray-600 px-4">
                <div>
...
export default PendingClaimsAwardedForPaymentsManagerReview;
