...
// Lists
import ListPendingClaimsAwardedPaymentsManagerReview
  from '../../components/forms/ListPendingClaimsAwardedPaymentsManagerReview';
import ListOnHoldClaimsAwardedPaymentsManagerReview
  from '../../components/forms/ListOnHoldClaimsAwardedPaymentsManagerReview';
import ListApprovedClaimsAwardedPaymentsManagerReview
  from '../../components/forms/ListApprovedClaimsAwardedPaymentsManagerReview';
import ListRejectedClaimsAwardedPaymentsManagerReview
  from '../../components/forms/ListRejectedClaimsAwardedPaymentsManagerReview';
...
const PaymentsManagerDashboard: React.FC = () => {
 ...
 //     items: [
  ...
  const handleMenuItemClick = (menu: string, item: string) => {
    if (normalize(menu) === 'payments') {
      if (normalize(item) === 'pending payments for review') setCurrentView('pending-review');
      else if (normalize(item) === 'onhold payments for review') setCurrentView('onhold-review');
      else if (normalize(item) === 'approved for payments') setCurrentView('approved');
      else if (normalize(item) === 'rejected from payments') setCurrentView('rejected');
  ...
  const loadCardCounts = async () => {
    ...
    const [p, h, a, r] = await Promise.all([
      getCount('pending_awarded_claims_paymentsmanagerreview_view'),
  ...
  const loadCharts = async () => {
    ...
    const ct = { StateInsured: empty12(), PrivateInsured: empty12(), PrivateUninsured: empty12() };
    const it = { Injury: empty12(), Death: empty12() };
  ...
  const renderCurrentView = () => {
    if (currentView === 'pending-review') {
      return (
        <div className="card p-4">
          <h2 className="text-xl font-semibold mb-4">Pending Payments — Manager Review</h2>
          <ListPendingClaimsAwardedPaymentsManagerReview onClose={() => setCurrentView(null)} />
        </div>
      );
    }

    if (currentView === 'onhold-review') {
      return (
        <div className="card p-4">
          <h2 className="text-xl font-semibold mb-4">On Hold — Manager Review</h2>
          <ListOnHoldClaimsAwardedPaymentsManagerReview onClose={() => setCurrentView(null)} />
        </div>
      );
    }
    ...
    if (currentView === 'approved') {
      return (
        <div className="card p-4">
          <h2 className="text-xl font-semibold mb-4">Approved For Payments</h2>
          <ListApprovedClaimsAwardedPaymentsManagerReview onClose={() => setCurrentView(null)} />
        </div>
      );
    }
    ...
    if (currentView === 'rejected') {
      return (
        <div className="card p-4">
          <h2 className="text-xl font-semibold mb-4">Rejected From Payments</h2>
          <ListRejectedClaimsAwardedPaymentsManagerReview onClose={() => setCurrentView(null)} />
        </div>
      );
    }

    // Default dashboard (clean)
    return (
      <>
        {/* Year Switcher */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <SvgLineChart
            ...
            labels={monthLabels}
            series={[{ name: 'Total', data: monthlyTotals, color: '#2563eb' }]}
